...
// NS Version has No Screen updates to save CPU
...
int[] averageImage(PImage im){
  ...
  for (int y = 0; y < im.pixels.length; y++){
    avRed += red(im.pixels[y]);
    avGreen += green(im.pixels[y]);
    avBlue += blue(im.pixels[y]);
  }
  avRed = avRed/im.pixels.length;
  avGreen = avGreen/im.pixels.length;
  avBlue = avBlue/im.pixels.length;

  returnArray[0] = avRed;
  returnArray[1] = avGreen;
  returnArray[2] = avBlue;
...
void draw()
{
  ...
  // rObj is for OSC
  rObj = new Object[numX + 1];
  gObj = new Object[numX + 1];
  bObj = new Object[numX + 1];
  ...
  for(int j=0; j<numY; j++) {
    ...
    rObj[0] = new Integer(j);
    gObj[0] = new Integer(j);
    bObj[0] = new Integer(j);
    // loop the cols
    for(int i=0; i<numX; i++) {
      //grab a block of the camera image and average it
      cArray = averageImage(camera.get((i*blockSizeX)-blockSizeX/2,(j*blockSizeY)-blockSizeY/2, blockSizeX, blockSizeY));
      r = cArray[0];
      g = cArray[1];
      b = cArray[2];
...
//      rect((i*displayBlockX),(j*displayBlockY), displayBlockX, displayBlockY);
      ...
      rObj[i + 1] = new Integer(r);
      gObj[i + 1] = new Integer(g);
      bObj[i + 1] = new Integer(b);
    }
    // send the rows as osc
    OscMessage myMessageR = new OscMessage("/txcyclosc/redrow");
    myMessageR.add(rObj); 
    /* send the message */
    oscP5.send(myMessageR, myRemoteLocation); 
    OscMessage myMessageG = new OscMessage("/txcyclosc/greenrow");
    myMessageG.add(gObj); 
    /* send the message */
    oscP5.send(myMessageG, myRemoteLocation); 
    OscMessage myMessageB = new OscMessage("/txcyclosc/bluerow");
    myMessageB.add(bObj); 
    /* send the message */
    oscP5.send(myMessageB, myRemoteLocation); 
//  println(myMessage);  } 
}
}
