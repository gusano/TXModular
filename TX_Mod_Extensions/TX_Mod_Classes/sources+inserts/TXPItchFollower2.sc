...
init {arg argInstName;
	...
	synthDefFunc = { arg out, audioIn, sendPitch, sendPitch2, i_minFreq, i_maxFreq, lag, lagMin, lagMax, modLag;
		...
		SendTrig.kr((sendPitch2 * Impulse.kr(10)), holdTrigID2, freq); // sends current pitch if requested
		Out.kr(out, [
	...
	guiSpecArray = [
		...
		["DividingLine"],
		["NextLine"],
		["TXStaticText", "Pitch Min", {holdPitchMin.asString}, {arg view; holdPitchMinView = view.textView}, 200], 
		["TXCheckBox", "Detect min & max", "sendPitch2", 250],
		["NextLine"],
		["TXStaticText", "Pitch Max", {holdPitchMax.asString}, 
			{arg view; holdPitchMaxView = view.textView; this.resetMinMaxPitchDisplay}, 200], 
		["ActionButton", "Reset", {this.resetMinMaxPitchDisplay}, 150, TXColor.white, TXColor.sysDeleteCol],
...
resetMinMaxPitchDisplay {arg argPitch;
	holdPitchMin = 999999;
	holdPitchMax = 0;
	{holdPitchMinView.string = " ";}.defer;
	{holdPitchMaxView.string = " ";}.defer;
}
