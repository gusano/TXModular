...
init {arg argInstName;
	...
	arrActionSpecs = this.buildActionSpecs([
		...
		["TXPopupAction", "Loop", {["No Loop"]++system.loopBankStereo
		.collect({arg item, i; 
	//		item.at(0).keep(-50);
			item.at(0).basename;
		})},
...
buildGuiSpecArray {
	...
	if (displayOption == "showLoop", {
		guiSpecArray = guiSpecArray ++[
			["TXPopupAction", 
				"Stereo loop", {["No Loop"]++system.loopBankStereo
				.collect({arg item, i; 
			//		item.at(0).keep(-50);
					item.at(0).basename;
				})},
...
loadLoop { arg argIndex; // method to load loops into buffer
	...
	Routine.run {
		...
		if (argIndex == 0, {
			...
			holdLoopInd = (argIndex - 1).min(system.loopBankStereo.size-1);
			holdBuffer = Buffer.read(system.server, system.loopBankStereo.at(holdLoopInd).at(0), 
				action: { arg argBuffer; 
					{
					//	if file loaded ok
						if (argBuffer.notNil, {
							this.setSynthArgSpec("bufnumLoop", argBuffer.bufnum);
							loopFileName = system.loopBankStereo.at(holdLoopInd).at(0);
							loopNumChannels = argBuffer.numChannels;
							loopTotalBeats = system.loopBankStereo.at(holdLoopInd).at(1);
							...
							TXInfoScreen.new("Invalid Loop File" 
							  ++ system.loopBankStereo.at(holdLoopInd).at(0));
						});
