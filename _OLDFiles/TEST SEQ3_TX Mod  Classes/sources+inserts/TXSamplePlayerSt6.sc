		...
		[	["Single shot", 
					...
					if (this.getSynthArgSpec("sampleIsStereo") == 1, {
						output = BufRd.ar(2, bufnumSampleStereo, 
							(Sweep.ar(1, outRate * BufSampleRate.kr(bufnumSampleStereo))
								+ (((start * outRate.sign.max(0)) + (end * outRate.sign.neg.max(0))) * BufFrames.kr(bufnumSampleStereo))
							)
						...
						output = BufRd.ar(1, bufnumSampleMono, 
							(Sweep.ar(1, outRate * BufSampleRate.kr(bufnumSampleMono))
								+ (((start * outRate.sign.max(0)) + (end * outRate.sign.neg.max(0))) * BufFrames.kr(bufnumSampleMono))
							)
			...
			["Looped", 
				{arg outRate, bufnumSampleStereo, bufnumSampleMono, start, end; 
					...
					if (this.getSynthArgSpec("sampleIsStereo") == 1, {
						output = BufRd.ar(2, bufnumSampleStereo, 
							Phasor.ar(0, outRate * BufRateScale.kr(bufnumSampleStereo), start * BufFrames.kr(bufnumSampleStereo), 
								end * BufFrames.kr(bufnumSampleStereo))
						);
					}, {
						output = BufRd.ar(1, bufnumSampleMono, 
							Phasor.ar(0, outRate * BufRateScale.kr(bufnumSampleMono), start * BufFrames.kr(bufnumSampleMono), 
...
//invalid		["step", 'step'],
	...
	synthDefFunc = { 
		arg out, gate, note, velocity, keytrack, transpose, pitchbend, pitchbendMin, pitchbendMax, 
			...
			sampleNo, sampleFreq, start, end, reverse, level, 
	...
	arrActionSpecs = this.buildActionSpecs([
		...
		["TXPopupActionPlusMinus", "Sample", {["No Sample"]++system.sampleBankFileNames},
...
loadSample { arg argIndex; // method to load samples into buffer
	var holdBuffer, holdBufferNum, holdBufferString, holdSampleInd, holdModCondition;
	Routine.run {
		...
		this.resetPlayRange;
		// adjust index
		holdSampleInd = (argIndex - 1).min(system.sampleBank.size-1);
		// check for invalid samples
		if (argIndex == 0 or: {system.sampleBank.at(holdSampleInd).at(3) == false}, {
			...
			// check for stereo
			if (system.sampleBank.at(holdSampleInd).at(2) == 2, {
			...
			holdBuffer = Buffer.read(system.server, system.sampleBank.at(holdSampleInd).at(0), 
				action: { arg argBuffer; 
					{
					//	if file loaded ok
						if (argBuffer.notNil, {
							this.setSynthArgSpec(holdBufferString, argBuffer.bufnum);
							sampleFileName = system.sampleBank.at(holdSampleInd).at(0);
							sampleNumChannels = argBuffer.numChannels;
							sampleFreq = system.sampleBank.at(holdSampleInd).at(1);
							// store Freq to synthArgSpecs
							this.setSynthArgSpec("sampleFreq", sampleFreq);
						},{
							buffers.at(0).zero;
							...
							TXInfoScreen.new("Invalid Sample File" 
							  ++ system.sampleBank.at(holdSampleInd).at(0));
						});
						//	rebuild synth to update stereo/mono 
						this.rebuildSynth;
