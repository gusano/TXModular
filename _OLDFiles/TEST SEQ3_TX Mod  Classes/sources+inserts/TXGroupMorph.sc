...
init {arg argInstName;
	...
	arrOptionData = [
		[
			["Group Morph", 
				{arg groupA, groupB, groupC, groupD, sliderXValTotal, sliderYValTotal;
		...
		//		GroupD = bottom right/x=1,y=1
				groupLeft = (groupA * (1-sliderYValTotal)) + (groupC * sliderYValTotal);
				groupRight = (groupB * (1-sliderYValTotal)) + (groupD * sliderYValTotal);
				groupOut = (groupLeft * (1-sliderXValTotal)) + (groupRight * sliderXValTotal);
				}
			],
			["Group A Only", 
				{arg groupA, groupB, groupC, groupD; groupA; }
			],
			["Group B Only", 
				{arg groupA, groupB, groupC, groupD; groupB; }
			],
			["Group C Only", 
				{arg groupA, groupB, groupC, groupD; groupC; }
			],
			["Group D Only", 
				{arg groupA, groupB, groupC, groupD; groupD; }
			],
		]
	];
	arrSynthArgSpecs = [
	...
	synthDefFunc = { 
		...
		var groupA, groupB, groupC, groupD, sliderXValTotal, sliderYValTotal, outFunction; 
		...
		sliderXValTotal = (sliderXVal + modSliderXVal).max(0).min(1);
		sliderYValTotal = (sliderYVal + modSliderYVal).max(0).min(1);
				
		outFunction = arrOptionData.at(0).at(arrOptions.at(0)).at(1);

		
		Out.kr(out, outFunction.value(groupA, groupB, groupC, groupD, sliderXValTotal, sliderYValTotal));
	};
...
buildGuiSpecArray {
	...
	if (displayOption == "showGroupMorph", {
		guiSpecArray = guiSpecArray ++[
			["SynthOptionPopupPlusMinus", "Output", arrOptionData, 0], 
			["SpacerLine", 20], 
			["TextBarLeft", "Group C", 80, 24, TXColor.grey7],
			["Spacer", 250], 
			["TextBarLeft", "Group D", 80, 24, TXColor.grey7],
			["SpacerLine", 2], 
			["TX2DSlider", "X-Y Morph", ControlSpec(0, 1), "sliderXVal", "sliderYVal", nil, 250, 334],
			["SpacerLine", 2], 
			["TextBarLeft", "Group A", 80, 24, TXColor.grey7],
			["Spacer", 250], 
			["TextBarLeft", "Group B", 80, 24, TXColor.grey7],
		];
	});
