...
TXNoteSequencer3 : TXModuleBase {		// Sequencer module 
...
//		["seqResetStep", 64],	// not used now
	...
	seqCurrentStep = 0;
	seqRunning = false;
	this.updateTriggerStatus(" Ready to trigger first step");
// not used now
//	seqResetCounter = 0;
	...
	guiSpecTitleArray = [
		["TitleBar"], 
		["ActionButtonBig", "Start", 
			{this.startSequencer; 
				system.showViewIfModDisplay(this);
			}, 40, nil, TXColor.sysGuiCol2], 
		["ActionButtonBig", "Stop", 
			{triggerModeRunning = false; this.stopSequencer; this.prepareToStartSeq;
				system.showViewIfModDisplay(this);}, 40, nil, TXColor.sysGuiCol2], 
...
buildGuiSpecArray {
	...
	// if seq is playing or in Manual Trigger Mode restrict displayOption
	if (seqRunning == true or: (triggerModeRunning == true), {
		if (this.getSynthArgSpec("displayOption") != "showOutputs", {
			this.setSynthArgSpec("displayOption", "showBPMOptions");
		});
	});
	// create guiSpecArray
	guiSpecArray = [
	...
	// if seq not playing and not in Manual Trigger Mode show other buttons
	if (seqRunning == false and: (triggerModeRunning == false), {
	...
	if (seqRunning == true, {
		guiSpecArray = guiSpecArray ++[
			["SpacerLine", 8], 
			["TextBar", "NOTE:  Sequencer is playing - sequence steps cannot be edited.", 600, 30],
			["SpacerLine", 2], 
			["DividingLine"], 
			["SpacerLine", 2], 
		];
	...
	if (triggerModeRunning == true, {
		guiSpecArray = guiSpecArray ++[
			["SpacerLine", 8], 
			["TextBar", "NOTE:  Manual Trigger Mode is on - sequence steps cannot be edited. Press Stop button to end this mode ", 600, 30],
			["SpacerLine", 2], 
			["DividingLine"], 
			["SpacerLine", 2], 
		];
...
//			["SeqSelectFirstDisplayStep", 16, "displayFirstStep", 64], // now scrolling used
			...
			["DividingLine"], 
			["SpacerLine", 2], 
			["TXCheckBox", "Output Control 1 values for each step", "midiControl1On", nil, 300],
			["NextLine"], 
			["EZSlider", "1 - Controller", ControlSpec(0, 127, step: 1), "midiControlNo1"], 
			["SpacerLine", 2], 
			["DividingLine"], 
			["SpacerLine", 2], 
			["TXCheckBox", "Output Control 2 values for each step", "midiControl2On", nil, 300],
			["NextLine"], 
			["EZSlider", "2 - Controller", ControlSpec(0, 127, step: 1), "midiControlNo2"], 
			["SpacerLine", 2], 
			["DividingLine"], 
			["SpacerLine", 2], 
			["TXCheckBox", "Output Control 3 values for each step", "midiControl3On", nil, 300],
			["NextLine"], 
			["EZSlider", "3 - Controller", ControlSpec(0, 127, step: 1), "midiControlNo3"], 
		];
	});
	...
	if (this.getSynthArgSpec("displayOption") == "showBPMOptions", {
		// if triggerModeRunning show Manual controls
		if (triggerModeRunning == true, {
			guiSpecArray = guiSpecArray ++[
				["SpacerLine", 10], 
				["TextBar", "Manual Controls", 150],
				["SpacerLine", 2], 
				["TXStaticText", "Next step", {this.triggerStatus}, {arg view; triggerStatusView = view.textView}, 378], 
				["SpacerLine", 2], 
		...
		// if seq not playing and not in Manual Trigger Mode show Play Mode popup
		if (seqRunning == false and: (triggerModeRunning == false), {
			guiSpecArray = guiSpecArray ++[
				["SpacerLine", 10], 
				["TXPopupActionPlusMinus", "Play Mode", 
		...
		// if not triggerModeRunning show BPM
		if (triggerModeRunning == false, {
			guiSpecArray = guiSpecArray ++[
				["SpacerLine", 10], 
				["TXMinMaxSliderSplit", "BPM", ControlSpec(1, 900), "seqBPM", "seqBPMMin", "seqBPMMax",
		...
		guiSpecArray = guiSpecArray ++[
			...
			["TXPopupAction", "Next pattern", 
				["None", "Repeat current pattern", "Use pattern chain - in chain step order",
				 ...
				 "nextPatternInd", {this.buildGuiSpecArray; this.prepareToStartSeq; system.showView;}, 400],
		];
...
updateTriggerStatus { arg argStatus;
	triggerStatus = argStatus ? (" Ready to trigger next Step: " 
