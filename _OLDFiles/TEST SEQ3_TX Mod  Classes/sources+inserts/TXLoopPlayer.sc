...
init {arg argInstName;
	...
	arrActionSpecs = this.buildActionSpecs([
		...
		["TXPopupActionPlusMinus", 
			"Loop", {["No Loop"]++system.loopBankMonoFileNames},
...
buildGuiSpecArray {
	...
	if (displayOption == "showLoop", {
		guiSpecArray = guiSpecArray ++[
			// array of loop filenames - beginning with blank loop  - only show mono files
			["TXPopupActionPlusMinus", "Mono loop", {["No Loop"]++system.loopBankMonoFileNames},
				"loopNo", { arg view; 
...
loadLoop { arg argIndex; // method to load loops into buffer
	...
	Routine.run {
		...
		system.server.sync;
		// adjust index
		holdLoopInd = (argIndex - 1).min(system.loopBankMono.size-1);
		// check for invalid samples
		if (argIndex == 0 or: {system.loopBankMono.at(holdLoopInd).at(3) == false}, {
			...
			holdBuffer = Buffer.read(system.server, system.loopBankMono.at(holdLoopInd).at(0), 
				action: { arg argBuffer; 
					{
					//	if file loaded ok
						if (argBuffer.notNil, {
							this.setSynthArgSpec("bufnumLoop", argBuffer.bufnum);
							loopFileName = system.loopBankMono.at(holdLoopInd).at(0);
							loopNumChannels = argBuffer.numChannels;
							loopTotalBeats = system.loopBankMono.at(holdLoopInd).at(1);
							...
							TXInfoScreen.new("Invalid Loop File" 
							  ++ system.loopBankMono.at(holdLoopInd).at(0));
						});
