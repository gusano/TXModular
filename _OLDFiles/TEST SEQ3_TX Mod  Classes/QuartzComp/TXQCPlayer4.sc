...
init {arg argInstName;
	...
	synthDefFunc = { 
		arg out, i_screenWidth, i_screenHeight, i_posX, i_posY, i_borderX, i_borderY, i_maxFPS, i_winControls, 
	...
	if ( (qcFileName == "" and: argPath.isNil).not, {   // only show screen if valid path
		{
			...
			if (holdQCWindow.isNil) {
				...
				if (File.exists(holdPath), {
					// make window
					if (this.getSynthArgSpec("i_winControls") == 1, {holdWinBorder = true;}, {holdWinBorder = false;});
					holdQCWindow= SCWindow(" ", Rect(screenPosX, screenPosY, screenWidth, screenHeight), 
					...
					holdQCView = SCQuartzComposerView(holdQCWindow, 
						Rect(borderX ,borderY, (screenWidth-(borderX*2)).max(0), (screenHeight-(borderY*2)).max(0)));
...
openQCComp {	
	...
	CocoaDialog.getPaths({ arg paths;
		...
		if (this.isValidQCFile(firstPath), {
			...
			{this.initialiseQCParameters}.defer(0.1);
...
/*
For some reason, initialiseQCParameters is not working, 
Method call is commented out above
*/

initialiseQCParameters {
	arrInputs.do({arg item, i; 
