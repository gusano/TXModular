...
TXWidget {
	...
	var height, <>heightMin=2, <>heightMax=530, width, <>widthMin=2, <>widthMax=990;
	var <>layoutX, <>layoutY;
...
//	var onScreen=false;	// not sure if needed in new design?
	...
	*new { arg argLayoutX, argLayoutY, argHeight, argWidth, arrExtraArgs;
	...
	buildGui { arg window, offsetLeft, offsetTop, screenWidth, screenHeight;
	...
	height_ { arg argHeight;
		height = argHeight.max(heightMin).min(heightMax);
		this.updateViews;
	}	
	width_ { arg argWidth;
		width = argWidth.max(widthMin).min(widthMax);
		this.updateViews;
	}	
	...
	getTemplatePropertyList {
		...
		templateProps.remove(\arrActions);
...
TXWActionButton : TXWLabelBox {
	...
	*initClass{
	...
	buildGui { arg window, offsetLeft, offsetTop, screenWidth, screenHeight;
		...
		holdView.action = {this.performActions};
		holdView.font = Font(font, fontSize);
		arrViews = arrViews.add(holdView);
	...
	keyActivate {
		...
		if ((keyListen == 1) and: (keyChar != ""), {
			TXFrontScreen.addKeyDownActionFunction(
				{arg argChar;
					if (argChar.asString == keyChar, {this.performActions; });
				 });
		});
	}
...
TXWSlider : TXWidget {
	...
	*initClass{
	...
	buildGui { arg window, offsetLeft, offsetTop, screenWidth, screenHeight;
		...
		holdView.action = {arg view; this.performActions(holdControlSpec.map(view.value))};
...
//		onScreen = true;
		...
		system.addScreenUpdFunc(
			...
			{ arg argArray;
				...
				holdView.value = this.getControlSpec.unmap(this.getValue ? 0);
			}
		);
	}	
	...
	performActions { arg argValue, argArrActions;
		// use arrActions if argument is nil
		argArrActions = argArrActions ? arrActions;
			...
			if (holdModule != 0, {
				...
				if (holdActionSpecs.size > 0, {
					...
					if (holdActionText.notNil, {
						...
						// if text not found, use number but only select older actions with legacyType == 1
						holdAction = holdActionSpecs.select({arg item, i; item.legacyType == 1}).at(holdActionInd);
					});
					// value valueAction  with arguments
					holdAction.setValueFunction.value(argValue, holdVal2, holdVal3, holdVal4);
				});
			});
		});
		//	gui update
	...
	*initClass{
	...
	buildGui { arg window, offsetLeft, offsetTop, screenWidth, screenHeight;
		...
		holdView.action = {arg view; 
			this.performActions(holdControlSpec.map(view.x), arrActions);
			this.performActions(holdControlSpec2.map(view.y), arrActions2);
		};
	...
	*initClass{
	...
	buildGui { arg window, offsetLeft, offsetTop, screenWidth, screenHeight;
		...
		holdView.action = {arg view; 
			...
			this.performActions(view.value)
		};
	...
	*initClass{
	...
	buildGui { arg window, offsetLeft, offsetTop, screenWidth, screenHeight;
		...
		holdSliderView.action = {arg view; 
			this.performActions(holdControlSpec.map(view.value));
			// update numberview
		...
		holdNumberView.stringColor = knobColour;
...
//		onScreen = true;
		...
		system.addScreenUpdFunc(
			...
			{ arg argArray;
				...
				holdSliderView.value = this.getControlSpec.unmap(this.getValue ? 0);
				holdNumberView.value = (this.getValue ? 0).round(0.001);
			}
		);
	}	
...
TXWTextEditBox : TXWNumberBox {
	...
	*initClass{
	...
	buildGui { arg window, offsetLeft, offsetTop, screenWidth, screenHeight;
		...
		holdView.background = background;
		holdView.action = {arg view; 
			this.performActions(view.string)
		};
