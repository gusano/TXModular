...
*new{ arg message = "ERROR", showCloseBtn=1 , winColour, inLeft=20, inTop=500, arrInfoLines;	 ^super.new.makeWindow(message, showCloseBtn, winColour, inLeft, inTop, arrInfoLines);
} 
...
makeWindow{ arg message, showCloseBtn, winColour, inLeft, inTop, arrInfoLines;
	...
	w = Window("Information", Rect(inLeft, inTop, 840, 150 + arrInfoLinesHeight));
	w.front;
...
makeConfirmWindow{ arg argConfirmedAction, message, winColour, inLeft, inTop;
	...
	w = Window("Information", Rect(inLeft, inTop, 840, 150));
	w.front;
	w.alwaysOnTop_(true);
	w.view.decorator = FlowLayout(w.view.bounds);
	w.view.background = winColour;
