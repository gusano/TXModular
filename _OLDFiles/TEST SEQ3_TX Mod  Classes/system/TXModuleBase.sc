...
/*
	...
	classvar <moduleType;			// "action"/ "source"/ "groupsource"/ "insert"/ "bus"/"channel"
...
//		"wetDryMix" & "modWetDryMix" - for  audio insert FX modules
//	
...
// ========================================================================
	...
	var <>arrOptions;			// for storing and recalling synth option
...
baseInit {arg argModule, argInstName; 
	...
	if (argModule.class.moduleType == "bus", { 
		...
		arrCtlSCInBusses = argModule.class.arrCtlSCInBusSpecs.collect({ arg item, i;
			Bus.control(system.server, item.at(1));
		});
...
copyToClipboard {
	system.setModuleClipboard(this.class, this.saveData);
} 
...
loadSynthDef {
	...
	if (synthDefFunc.notNil, {	
		...
		//	load the SynthDef
	//	SynthDef(instName, synthDefFunc, synthDefRates).load(system.server);
	// N.B. line above commented out - seemed to be using old versions of synthdef when load was used
	// for now just SEND the SynthDef don't LOAD it. 
...
// testing - code for tracing module UGen ins/outs after building
//	
//	if (this.instName.keep(6) == "Filter", 
//	{
//	moduleNode.trace;
//	});
...
midiNoteInit { 
	...
	midiNoteStatus = true;
	midiNotes = Array.newClear(128);  // array has one slot per possible MIDI note
}
...
midiNoteActivate { 
	...
	midiNoteOnResp = NoteOnResponder ({  |src, chan, num, vel|
		...
		, {
			// if current not already playing then end
			if (midiNotes[num].notNil, {
				midiNotes[num].set(\gate, 0);
			});
	...
	midiNoteOffResp = NoteOffResponder({  |src, chan, num, vel|
		...
		if (midiNotes[num].notNil,{
			if (midiNotes[num].isPlaying, {
				midiNotes[num].set(\gate, 0); 
			});
		});
		midiNotes.put(num, nil);	//  free 
	});
}
...
createSynthNote { arg argNote=60, argVeloc=100, argEnvTime=1, seqLatencyOn=1;
	...
	, {
		...
		holdSynth = Synth.new(instName,
			arrSynthArgs ++ [\note, argNote, \velocity, argVeloc, \envtime, argEnvTime], 
			moduleNode.nodeID, 
			\addToTail ;
		);
		// register node
		...
		if (midiNoteStatus == true, {
			// save it to free later
			midiNotes.put(argNote, holdSynth);	
		});
...
releaseSynthGate { arg argNote=60;
	// release node
	if (midiNotes[argNote].notNil,{
		if (midiNotes[argNote].isPlaying, {
			midiNotes[argNote].set(\gate, 0); 
		});
	});
	midiNotes.put(argNote, nil);	//  free 
}
