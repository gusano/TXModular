...
TXSamplePlayerSt3 : TXModuleBase {
...
//			]
	...
	synthDefFunc = { 
		arg out, gate, note, velocity, keytrack, transpose, pitchbend, pitchbendMin, pitchbendMax, 
			...
			envtime=0, delay, attack, decay, sustain, sustainTime, release, curve, timeMultiply, intKey, 
			modStart, modEnd, modReverse, modPitchbend, modDelay, modAttack, modDecay, 
			modSustain, modSustainTime, modRelease, modCurve;
		var outEnv, outFreq, intonationFunc, pbend, outRate, outFunction, outSample, curveAdjusted, sStart, sEnd, rev, 
		...
		outFreq = (intonationFunc.value((note + transpose), intKey) * keytrack) + ((sampleFreq.cpsmidi + transpose).midicps * (1-keytrack));
