...
TXNoteSequencer2 : TXModuleBase {		// Sequencer module 
	...
	classvar	<guiTop=300;
	classvar	<seqLatency = 0.1;	// all sequencers use same latency for server timing.
...
//	seqResetCounter = 0;
	...
	guiSpecTitleArray = [
		["TitleBar"], 
		["ActionButtonBig", "Start", {this.startSequencer}, 40, nil, TXColor.sysGuiCol2], 
		["ActionButtonBig", "Stop", {this.stopSequencer}, 40, nil, TXColor.sysGuiCol2], 
	...
	arrActionSpecs = this.buildActionSpecs([
		...
		["TXCheckBox", "Midi learn notes and velocities", "midiLearn", 
			{arg view; this.midiLearn(view.value);}, 450],
		...
		["TXCheckBox", "Randomise step order every cycle", "randomStepOrder", 
			{this.rebuildStepOrderArr;}, 450],
		...
		seqClock.schedAbs(seqClock.elapsedBeats,{	
			...
			if (	// if step is to be played 
				this.getSynthArgSpec("arrOnOffSteps").at(holdStepNo) == 1
					...
					and: ((this.getSynthArgSpec("arrProbabilities").at(holdStepNo)/100 - rand(1.0))
						.ceil == 1)
			...
			if (seqCurrentStep > (this.getSynthArgSpec("seqEndStep") - 1), { 
				...
				if ((this.getSynthArgSpec("nextPatternInd") == 2), {
					...
					if (stopSeqFlag == false, {
						// load new slot
						holdSlotNo = this.getSynthArgSpec("arrChainSlots")
							.at(this.getSynthArgSpec("chainCurrentStep") - 1);
	...
	arrProcessSpecs = [
		["select a process...", 
			...
			"Randomise note length Y values","Randomise probability values","Randomise delay values"
				...
				["ActionButton", item, 
					{this.setSynthArgSpec("processTypeInd", i + 1); this.buildGuiSpecArray;
						system.showView;}, 
