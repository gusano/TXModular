...
TXSamplePlayerSt4 : TXModuleBase {
...
//invalid		["step", 'step'],
	...
	arrActionSpecs = this.buildActionSpecs([
		...
		["TXPopupAction", "Sample", {["No Sample"]++system.sampleFilesStereo
		.collect({arg item, i; 
	//		item.at(0).keep(-50);
			item.at(0).basename;
		})},
...
loadSample { arg argIndex; // method to load samples into buffer
	...
	Routine.run {
		...
		if (argIndex == 0, {
			...
			holdSampleInd = (argIndex - 1).min(system.sampleFilesStereo.size-1);
			holdBuffer = Buffer.read(system.server, system.sampleFilesStereo.at(holdSampleInd).at(0), 
				action: { arg argBuffer; 
					{
					//	if file loaded ok
						if (argBuffer.notNil, {
							this.setSynthArgSpec("bufnumSample", argBuffer.bufnum);
							sampleFileName = system.sampleFilesStereo.at(holdSampleInd).at(0);
							sampleNumChannels = argBuffer.numChannels;
							sampleFreq = system.sampleFilesStereo.at(holdSampleInd).at(1);
							// store Freq to synthArgSpecs
							this.setSynthArgSpec("sampleFreq", sampleFreq);
						},{
							buffers.at(0).zero;
							...
							TXInfoScreen.new("Invalid Sample File" 
							  ++ system.sampleFilesStereo.at(holdSampleInd).at(0));
						});
