...
TXNoteSequencer : TXModuleBase {		// Sequencer module 
...
//	seqResetCounter = 0;
	...
	arrActionSpecs = this.buildActionSpecs([
		...
		["EZNumber", "Note base", ControlSpec(0, 127, 'lin', 1, 48), "seqNoteBase"],		["TXCheckBox", "Mute output", "muteSeq", nil, 450],
		...
		["valueActionNumber", "Set Group Rand delay: value, group", [ControlSpec(-99, 100, default: 0), ControlSpec(1, 8, step: 1, default: 1)],
...
buildGuiSpecArray {
	...
	if (this.getSynthArgSpec("displayOption") == "showGlobal", {
		guiSpecArray = guiSpecArray ++[
			...
			["DividingLine", 500], 
			["SpacerLine", 1], 
			["TXCheckBox", "Midi learn notes/velocities: turn off to display values", 
				"midiLearn", {arg view; this.midiLearn(view.value);}, 500],
			["DividingLine", 500], 
			["SpacerLine", 1], 
	...
	if (this.getSynthArgSpec("displayOption") == "showStepNoteLengths", {
		guiSpecArray = guiSpecArray ++[
			...
			["TXMultiNumber", " (in beats) / Y", ControlSpec(0.1, 999), "arrNoteLengthsY", 16, 
				{this.updateSlot;}, "displayFirstStep"],
			["SpacerLine", 1], 
			["TXPopupAction", "Presets", arrStepNoteLenPresets.collect({arg item, i; item.at(0);}), "stepNoteLenPresetInd", 
...
//				["TextBarLeft", "WARNING - Do not edit the sequencer while it is playing using pattern chain"],
			];
		});
	});
